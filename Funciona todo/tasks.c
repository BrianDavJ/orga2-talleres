...
static int8_t create_task(tipo_e tipo) {
  ...
  //Pregunta si hay una entrada de la gdt correspondiente a las tss que este libre(se le pueda cargar un descriptor de tss)
  for (gdt_id = GDT_TSS_START; gdt_id < GDT_COUNT; gdt_id++) {
    //Si existe una entrada de la gdt libre entonces salgo del for y gdt_id es el indice de la gdt que corresponde a la entrada libre
    if (gdt[gdt_id].p == 0) {
      break;
    }
  }

  //Si la entrada no es menor a gdt_count(la cantidad maxima de entradas que puede tener nuestra gdt) para el kernel y tira ese mensaje
  kassert(gdt_id < GDT_COUNT, "No hay entradas disponibles en la GDT");

  //Agrega la tarea al scheduler y devuelve el id del scheduler que le corresponde.
  int8_t task_id = sched_add_task(gdt_id << 3);   //gdt_id == selector de segmento del tss descriptor de esa tarea porque dpl = 0 y u/s = 0 (system)
                                                  //task_id = un numero del 0 al 3 (scheduler solo para 4 tareas).

  //Crea las estructuras de paginacion para una tarea de usuario y carga la tss creada en la estructura tss_tasks (lista de tss)
  tss_tasks[task_id] = tss_create_user_task(task_code_start[tipo]);
                                                                    //Recibe la direccion fisica en la que empieza la pagina de codigo de la tarea

  //Crea el descriptor de tss de esta tarea y lo carga en la gdt.
  gdt[gdt_id] = tss_gdt_entry_for_task(&tss_tasks[task_id]);
                                                              //recibe como parametro un puntero a la tarea que esta en la estructura
                                                              //tss_tasks, pueden haber solo 4 tareas.
