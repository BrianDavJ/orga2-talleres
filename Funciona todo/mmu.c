...
paddr_t mmu_init_kernel_dir(void)
{
  ...
  //Se mapean las 1024 entradas de la tabla de paginas es decir al terminar esta funcion queda inicializada una entrada
  //del directorio de tabla de paginas y las 1024 entras de tabla de pagina correspondientes a esa entrada del directorio
  //es decir el total de memoria fisica que queda mapeada es 4kb == una pagina

  zero_page(KERNEL_PAGE_DIR);     //Limpio la pagina que empieza en 0x25000
  zero_page(KERNEL_PAGE_TABLE_0); //Limpio la pagina que empieza en 0x26000

  //Seteo la primera entrada del directorio de tabla de paginas
  kpd[0].attrs = 0x3; //Present y RW
  kpd[0].pt = ((uint32_t)KERNEL_PAGE_TABLE_0) >> 12;  //En realidad queda guardado solo un indice no la direccion.
  
  for (int i = 0; i < 1024; i++)
  {
    kpt[i].attrs = 0x3; //Present y RW
    kpt[i].page = i;    //Fuscking indice
  }
...
{
  pd_entry_t *pd =  CR3_TO_PAGE_DIR(cr3);           //Puntero a la base del directorio de tabla de pagians que quiero mappear
  uint32_t directorio_index = VIRT_PAGE_DIR(virt);  //Entrada del directorio que contiene la tabla
  uint32_t tabla_index = VIRT_PAGE_TABLE(virt);     //Entrada de tabla que apunta a la dirección física
  
  if (!(pd[directorio_index].attrs & MMU_P)){       //Si la entrada del directorio de tabla de paginas no esta presente
    
    paddr_t nuevo_pt = mmu_next_free_kernel_page(); //Pido una pagina nueva para la tabla de paginas
    
    zero_page(nuevo_pt);                            //Limpio una pagina que empieza desde nuevo_pt
    
    pd[directorio_index].pt = (nuevo_pt >> 12);     //Focking indice
  }

  //|= es un bitwise or
  pd[directorio_index].attrs |= attrs;            //Asigno attrs a la entrada correspondiente del directorio de tabla de paginas
  pt_entry_t *pt = pd[directorio_index].pt << 12; //Direccion fisica en la que empieza la tabla de paginas correspondiente a esta entrada del directorio
  pt[tabla_index].attrs = attrs;                  //Asigno attrs a la entrada de la tabla de paginas
  pt[tabla_index].page = phy >> 12;               //Asigno indice al campo puntero
  tlbflush();                                     //Porque se modifico la estructora de paginacion
}
...
paddr_t mmu_init_task_dir(paddr_t phy_start)
{
  ...
  paddr_t code_virt = TASK_CODE_VIRTUAL;  // 0x08000000;
  paddr_t phy_end = phy_start;            //+PAGE_SIZE;
  
  paddr_t stack_virt = TASK_STACK_BASE;   //0x08003000;
  paddr_t stack_phy = mmu_next_free_user_page();

  paddr_t compartido_phy = SHARED;
  paddr_t compartido_virt = TASK_SHARED_PAGE; // stack_virt+PAGE_SIZE;
  ...
  mmu_map_page(cr3_actual, code_virt + PAGE_SIZE, phy_start + PAGE_SIZE, (MMU_U|MMU_P));
...
bool page_fault_handler(vaddr_t virt)
{
  ...
  if (virt>=ON_DEMAND_MEM_START_VIRTUAL && virt<=ON_DEMAND_MEM_END_VIRTUAL){
    ...
    {
      pd_entry_t* cr3=rcr3();
      mmu_map_page(cr3,virt,ON_DEMAND_MEM_START_PHYSICAL,MMU_W|MMU_P|MMU_U);
    }
    return 1;
  }
  return 0;
