...
int8_t sched_add_task(uint16_t selector) {
  ...
  for (int8_t i = 0; i < MAX_TASKS; i++) {
    ...
    if (sched_tasks[i].state == TASK_SLOT_FREE) {
      ...
      sched_tasks[i] = (sched_entry_t) {.selector = selector, .state = TASK_PAUSED,};
      return i;
    }
  }
...
uint16_t sched_next_task(void) {
  ...
  //Puede ocurrir que la unica tarea runnable sea l a taera actual, 
  //en ese caso next_task devuelve el id de la taria actual
